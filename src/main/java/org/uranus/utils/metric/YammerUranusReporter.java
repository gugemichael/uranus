 ...
 */
public class YammerUranusReporter extends AbstractReporter implements
                                                             MetricProcessor<YammerUranusReporter.Context> 
{
	class Context {
    ...
    public void processMeter(MetricName name, Metered meter, Context ctx) {
        ...
        ctx.result = String.format("%n    count = %,d%n    mean rate = %,2.0f %s/%s%n    1-minute rate = %,2.0f %s/%s%n    5-minute rate = %,2.0f %s/%s%n    15-minute rate = %,2.0f %s/%s", 
        					meter.count(),meter.meanRate(),meter.eventType(),unit, meter.oneMinuteRate(), meter.eventType(), unit, meter.fiveMinuteRate(), meter.eventType(),
        					unit, meter.fifteenMinuteRate(), meter.eventType(), unit);
    }

    @Override
    public void processHistogram(MetricName name, Histogram histogram, Context ctx) {
        final Snapshot snapshot = histogram.getSnapshot();
        ctx.result = String.format("%n    min = %,2.0f%n    max = %,2.0f%n	mean = %,2.0f%n    stddev = %,2.0f%n    median = %,2.0f%n    75%% <= %,2.0f%n    99%% <= %,2.0f", histogram.min(),
        		histogram.max(), histogram.mean(), histogram.stdDev(), snapshot.getMedian(), snapshot.get75thPercentile(), snapshot.get99thPercentile());
    }
    ...
    public void processTimer(MetricName name, Timer timer, Context ctx) {
        ...
        ctx.result += String.format("%n    min = %,2.0f%s%n    max = %,2.0f%s%n    mean = %,2.0f%s%n    stddev = %,2.0f%s%n    median = %,2.0f%s%n    75%% <= %,2.0f%s%n    99%% <= %,2.0f%s"
